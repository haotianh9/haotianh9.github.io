...
\begin{document}
	...
	\begin{resume}%
		...
		\begin{longtable}{@{}>{\raggedright}p{0.5\linewidth}
			...
			{\bf Citizenship:} People's Republic of China & {\bf Mobile:} +1 (213) 462 - 5919\\
			{\bf E-mail:} haotianh@usc.edu 
			% & {\bf Webpage:}   \url{https://haotianh9.github.io/haotian9.github.io/} (to be done in future) 
			\\
			{\bf Social media:}
		...
		\begin{longtable}{@{}>{\raggedright}p{0.07\linewidth}
			...
			\hspace{-4ex}{2020 - } & {\bf Flowtaxis via RL}, supervised by \textit{Prof.~Eva Kanso}, \textit{Dr.~Josh Merel} \\
			& \hspace{2ex} joint with \textit{Sina Heydari}, \textit{Yusheng Jiao}, \textit{Feng Ling}, \textit{Basile Radisson}\\
			& Using reinforcement learning to follow vortical wakes based on local flow sensory\\
		...
		\begin{longtable}{@{}>{\raggedright}p{0.07\linewidth}
			...
			& Study the role of active and passive flexion on swimming speed and efficiency of a self-propelling pitching plate using vortex sheet method \\
			& Parametric study on the effect of flexion phase, flexion angle and flexion ratio on swimming performance\\
		...
		\begin{longtable}{@{}>{\raggedright}p{0.07\linewidth}
				p{\dimexpr0.95\linewidth-2\tabcolsep-\arrayrulewidth\relax}@{}}
			\hspace{-4ex}{2016 - 2019} & {\bf Mechanisms of high generation in insects flight}, supervised by \textit{Prof.~Hong Liu} and \textit{Dr.~Yang Xiang}\\
			& \hspace{2ex} joint with \textit{Dr.~Suyang Qing}\\
			& Experimental study on the role of the phase difference on leading edge vortex formation using a scaled robotic flapping wing model in glycerin\\
		...
		\begin{longtable}{@{}>{\raggedright}p{0.07\linewidth}
			...
			& Construct a physics-based adaptive refinement method for finite-time Lyapunov exponent calculation\\
...
%		\vsrk\vsrk
		...
		\begin{longtable}{@{}>{\raggedright}p{0.07\linewidth}
				p{\dimexpr0.95\linewidth-2\tabcolsep-\arrayrulewidth\relax}@{}}
				& \textbf{at Coursera} \\
			\hspace{-4ex}{2021} & Build a Modern Computer from First Principles: From Nand to Tetris (Project-Centered Course), Hebrew University of Jerusalem, \emph{Prof.~Shimon Schocken} and \emph{Prof.~Noam Nisan} \\
			\hspace{-4ex}{2019} & {\bf Specialization}, DeepLearning.AI TensorFlow Developer , DeepLearning.AI, \emph{Prof.~Andrew Ng} (containing 4 courses) \\
			& {\bf Specialization}, Deep Learning, DeepLearning.AI, \emph{Prof.~Andrew Ng}, \emph{Prof.~Kian Katanforoosh}, and \emph{Prof.~Kian Katanforoosh} (containing 5 courses) \\
			& Machine Learning, Stanford University, \emph{Prof.~Andrew Ng} \\
			\hspace{-4ex}{2014} & General Chemistry, Peking University, \emph{Prof.~Jiang Bian} \\
